      ...
      <div className="rounded-3xl bg-white p-6 shadow-xl">
        ...
        <div className="mt-6 flex h-[60vh] flex-col gap-4">
          ...
          <form className="flex flex-col gap-3 sm:flex-row" onSubmit={handleSendMessage}>
            ...
            />
            <button type="submit" disabled={!isMatched || !input.trim()} className="rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white transition hover:bg-secondary disabled:cursor-not-allowed disabled:bg-gray-300">
              전송
            </button>
          </form>
          <div className="flex flex-wrap gap-3 text-sm">
            <button type="button" onClick={handleFindNext} className="rounded-full border border-primary px-4 py-2 font-semibold text-primary transition hover:bg-primary hover:text-white">
              다음 찾기
            </button>
          </div>
        </div>
