...
export class Matchmaker {
  ...
  registerRoom(roomId: string, clients: [ClientInfo, ClientInfo]) {
    ...
    clients.forEach((client) => {
      this.socketToRoom.set(client.socketId, roomId);
    });
  }
  ...
  getPartner(socketId: string): ClientInfo | null {
    ...
    if (!room) return null;
    return room.clients.find((client) => client.socketId !== socketId) ?? null;
  }
  ...
  endSession(socketId: string): RoomState | null {
    ...
    room.clients.forEach((client) => {
      this.socketToRoom.delete(client.socketId);
    });
    return room;
  }

  leaveQueue(socketId: string) {
    this.waiting = this.waiting.filter((client) => client.socketId !== socketId);
  }
